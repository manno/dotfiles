...
print_existing_diff() {
  ...
  cd skel
  find . -type f | grep -v $0 | grep -v '.vim/bundle/vundle' | grep -v '.gitkeep' | ruby -ne 'h=ENV["HOME"];f=$_.chomp.gsub(/\A\.\//,"");$_=%x(diff -q "#{f}" #{h}/#{f} 2>&1);if /differ/; puts "nvim -d skel/#{f} #{h}/#{f}"; elsif /diff:.*#{h}.*: No such file/; puts "cp skel/#{f} #{h}/#{f}"; end' | egrep -v 'nvim -d (skel/.bash_profile|skel/.gitconfig|skel/.profile)'
}
